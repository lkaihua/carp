//// This file is adapted from the Go standard library:
...
func (d Dir) Open(name string) (File, error) {
	// //CARP: change to use `fromFS` instead of `safefilepath.FromFS`
...
type anyDirs interface {
	...
	name(i int) string
	isDir(i int) bool
	modTime(i int) time.Time // //CARP: add file modification Time
	size(i int) int64        // //CARP: add file size
}
...
func (d fileInfoDirs) modTime(i int) time.Time { return d[i].ModTime() } // //CARP: add file modification Time
func (d fileInfoDirs) size(i int) int64        { return d[i].Size() }    // //CARP: add file size
...
// //CARP: add file modification Time
...
func dirList(w ResponseWriter, r *Request, f File) {
	...
	// //CARP: Instead of output html directly use a new format function
	...
	// }
	// fmt.Fprintf(w, "</pre>\n")
	// //CARP: Comment out the above and replace with the below
	outputDirList(w, r, dirs)
}
...
func checkIfMatch(w ResponseWriter, r *Request) condResult {
	...
	for {
		...
		// //CARP: Change to use `w.Header().Get` instead of `w.Header().get`
